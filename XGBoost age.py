...
def calculate_reweighing_weights(df, sensitive_attr, target_attr):
    ...
    """
    print(f"\n CALCULATING REWEIGHING WEIGHTS FOR AGE GROUPS (XGBoost)...")
...
print(f"\n XGBOOST MODEL INSIGHTS FOR AGE:")
...
if age_related_features:
    ...
    age_importance = feature_importance_baseline[
        feature_importance_baseline['feature'].isin(age_related_features)]
    if not age_importance.empty:
        print(age_importance.to_string(index=False))
    else:
        print("No age-related features found in top important features")
else:
    print(f"\nNo direct age features found in model (as expected - they were excluded)")
    print("Age patterns detected through correlated features via XGBoost gradient boosting")
...
df = apply_equal_opportunity_fairness_xgb(df, "baseline", "binary_age_group")

# ===== COMBINED APPROACH (Pre-processing + Post-processing) =====
print(f"\n COMBINED APPROACH (Pre-processing + Post-processing XGBoost)")
df = apply_equal_opportunity_fairness_xgb(df, "reweigh", "binary_age_group")
...
def analyze_fairness_comprehensive_xgb(df, group_column, approaches, title=""):
    ...
    print(f"\n BINARY AGE GROUP RISK ANALYSIS (XGBoost):")
    ...
    age_analysis.columns = ['Count', 'Default Rate', 'Baseline XGB Prob', 'Reweigh XGB Prob', 'Avg Loan Amount']
...
fairness_results = analyze_fairness_comprehensive_xgb(df, "binary_age_group", approaches,
                                                      "COMPREHENSIVE XGBOOST AGE FAIRNESS ANALYSIS")
...
print(f"\n XGBOOST ADVANTAGES FOR AGE FAIRNESS:")
print(f"{'=' * 55}")
...
print(f"• Built-in regularization prevents overfitting to age-related patterns")
print(f"• Early stopping ensures robust generalization across age groups")
...
print(f"• Maintains gradient boosting effectiveness while adjusting for age bias")
print(f"• Feature importance remains meaningful after reweighing")
print(f"• Robust convergence even with age-reweighted samples")

# ------------------ 14. Age-Specific Insights ------------------
print(f"\nBINARY AGE GROUP INSIGHTS (XGBoost Analysis):")
print(f"{'=' * 60}")
print(f"This XGBoost analysis compares lending fairness between two age cohorts:")
...
print(f"XGBoost excels at detecting complex age-related patterns through gradient boosting.")
...
print(f"  - Baseline XGB default probability: {above_35_data['y_prob_default_baseline'].mean():.3f}")
print(f"  - Reweigh XGB default probability: {above_35_data['y_prob_default_reweigh'].mean():.3f}")
for pred_col, approach_name in approaches:
...
print(f"  - Baseline XGB default probability: {below_35_data['y_prob_default_baseline'].mean():.3f}")
print(f"  - Reweigh XGB default probability: {below_35_data['y_prob_default_reweigh'].mean():.3f}")
for pred_col, approach_name in approaches:
...
print(f"\nXGBoost Intervention Effectiveness for Age Status:")
...
print(f"\nXGBoost Age Status Considerations:")
print(f"• Binary age grouping leverages XGB's gradient boosting capabilities")
...
print(f"• XGBoost excels at capturing socioeconomic age dynamics")

# ------------------ 16. Advanced XGBoost Fairness Insights ------------------
print(f"\n ADVANCED XGBOOST AGE FAIRNESS INSIGHTS:")
print(f"{'=' * 55}")

# Analyze feature importance differences between age groups
print(f"Feature Importance Analysis by Age Status:")
for group_name in sorted(df["binary_age_group"].unique()):
...
print(f"\n XGBOOST VS OTHER ALGORITHMS FOR AGE FAIRNESS:")
print(f"{'=' * 60}")

print(f"Expected Performance Ranking (for age fairness):")
print(f"1. XGBoost: Highest AUC, best probability calibration, superior gradient boosting")
print(f"   - Industry-leading performance for credit scoring with age fairness")
...
print(f"   - Linear assumptions may miss complex age interaction patterns")
print(f"   - Good regulatory transparency but lower predictive performance")

print(f"\nXGBoost Age Fairness Advantages:")
print(f"• Most sensitive to subtle age status bias patterns")
print(f"• Superior handling of complex socioeconomic age interactions")
print(f"• Best-in-class probability calibration for fairness interventions")
print(f"• Robust to overfitting while maintaining age fairness gains")
...
print(f"4. Phase 4: Continuous model retraining with age fairness constraints")

print(f"\nMonitoring Requirements:")
print(f"• Real-time age fairness metric tracking (DP and EO gaps)")
print(f"• Monthly age status bias audits")
print(f"• Feature importance drift detection for age patterns")
...
print(f"\n FINAL RECOMMENDATIONS FOR XGBOOST AGE FAIRNESS:")
print(f"{'=' * 65}")
...
print(f"\nRegulatory Compliance for Age Status (XGBoost):")
if best_fairness['eo_gap'] < 0.05:
    print(f" COMPLIANT: Equal opportunity gap < 5%")
    print(f"   Meets age status non-discrimination requirements")
else:
    print(f"NON-COMPLIANT: Equal opportunity gap ≥ 5%")
    print(f"   May face regulatory scrutiny for age status discrimination")

print(f"\n IMPLEMENTATION PRIORITY FOR XGBOOST AGE FAIRNESS:")
...
print(f"6. Consider ethical implications of age structure in automated lending")

# ------------------ 21. Legal and Ethical Considerations ------------------
print(f"\n AGE STATUS LEGAL AND ETHICAL CONSIDERATIONS:")
print(f"{'=' * 50}")

print(f"Legal Framework:")
print(f"• Equal Credit Opportunity Act (ECOA) age status protections")
print(f"• Age Discrimination in Employment Act (ADEA) - Employment context")
print(f"• Fair Credit Reporting Act (FCRA) - Credit reporting fairness")
print(f"• Consumer Financial Protection Bureau (CFPB) - Supervisory guidance")

print(f"\nEthical Considerations:")
print(f"• Avoiding age structure discrimination in financial services")
print(f"• Ensuring equal access regardless of age status")
print(f"• Balancing actuarial soundness with age fairness")
print(f"• Protecting younger and older applicants from systemic bias")

print(f"\nBest Practices:")
print(f"• Regular bias testing across age structures")
...
# ------------------ 22. Final Conclusion ------------------
print(f"\n FINAL CONCLUSION:")
print(f"XGBoost with comprehensive age fairness interventions provides the highest")
print(f"predictive performance while ensuring equitable treatment across age structures.")
print(f"The gradient boosting approach excels at detecting complex age-related patterns")
...
if best_fairness['eo_gap'] < 0.05 and best_fairness == best_profit:
    ...
    print(f"   - Compliant with age status non-discrimination regulations")
    ...
    print(f"   - Strong protection against age status discrimination claims")
else:
    ...
    print(f"   - May need legal review for age status compliance")

print(f"\n XGBOOST AGE FAIRNESS SUMMARY:")
print(f"This comprehensive analysis demonstrates XGBoost's superior capability")
print(f"for age-fair lending through gradient boosting, industry-leading")
